...
async function exportBusinessData(businessId) {
  ...
  const appointments = await dbAll(
    `SELECT id, type_id, title, client_name, client_email, date, time, duration_minutes, reminder_offset_minutes, location, notes, status, source, created_at
     FROM appointments
     WHERE business_id = ?
  ...
  return {
    ...
    settings: settings
      ? {
        ...
        reminder_mode: settings.reminder_mode === true || settings.reminder_mode === 1,
        workspace_mode: String(settings.workspace_mode || (settings.reminder_mode ? 'reminders' : 'appointments')).toLowerCase(),
    ...
    appointments: appointments.map((a) => ({
      ...
      reminder_offset_minutes: Number(a.reminder_offset_minutes == null ? 10 : a.reminder_offset_minutes),
...
async function importBusinessData(businessId, backup) {
  ...
  const mergedSettings = {
    ...
    reminder_mode: payload.settings.reminder_mode === true || payload.settings.reminder_mode === 1,
    workspace_mode: String(
      payload.settings.workspace_mode
      || (payload.settings.reminder_mode === true || payload.settings.reminder_mode === 1 ? 'reminders' : 'appointments')
    ).toLowerCase(),
  ...
  if (USE_POSTGRES) {
    ...
    try {
      ...
      await tx.query(
        `INSERT INTO business_settings (business_id, business_name, owner_email, timezone, reminder_mode, workspace_mode, open_time, close_time, business_hours_json)
        ...
        [
          ...
          Boolean(mergedSettings.reminder_mode),
          mergedSettings.workspace_mode,
      ...
      for (const a of payload.appointments) {
        ...
        await tx.query(
          `INSERT INTO appointments
           (business_id, type_id, title, client_name, client_email, date, time, duration_minutes, reminder_offset_minutes, location, notes, status, source)
          ...
          [
            ...
            Number(a.reminder_offset_minutes == null ? 10 : a.reminder_offset_minutes),
  ...
  const tx = db.transaction((input) => {
    ...
    db.prepare(
      ...
      Number(Boolean(input.settings.reminder_mode)),
      String(input.settings.workspace_mode || (input.settings.reminder_mode ? 'reminders' : 'appointments')).toLowerCase(),
