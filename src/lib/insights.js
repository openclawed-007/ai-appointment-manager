...
async function createInsights(date, businessId) {
  ...
  const [recentAllRows, pendingRow, settings] = await Promise.all([
    dbAll(
  ...
  const recentActiveRows = recentAllRows.filter((row) => String(row.status || '').toLowerCase() !== 'cancelled');
  const selectedDayRows = recentActiveRows
    .filter((row) => String(row.date || '') === date)
    .sort((a, b) => String(a.time || '').localeCompare(String(b.time || '')));
  const weekRows = recentActiveRows.filter((row) => {
    const rowDate = String(row.date || '');
    return rowDate >= date && rowDate <= to7Str;
  });
