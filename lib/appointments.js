...
const PUBLIC_BOOKING_WINDOW_START_MINUTES = 9 * 60;
const PUBLIC_BOOKING_WINDOW_END_MINUTES = 18 * 60;
...
async function getAvailableSlots({ businessId, date, durationMinutes }) {
  ...
  const lastStart = PUBLIC_BOOKING_WINDOW_END_MINUTES - requestedDuration;
  const slots = [];
  for (
    let slotStart = PUBLIC_BOOKING_WINDOW_START_MINUTES;
    slotStart <= lastStart;
